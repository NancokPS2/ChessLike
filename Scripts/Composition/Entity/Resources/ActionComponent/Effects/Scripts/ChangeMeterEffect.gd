extends ComponentActionResourceEffect
...
@export var other_stat_bonuses: Array[ComponentStatus.StatKeys]
@export_range(-1,1,2) var other_stat_bonuses_sign: int = 1
@export var other_stat_bonuses_multiplier: float = 1
...
func _affect_entity(entity: Entity3D):
	...
	for key: ComponentStatus.StatKeys in other_stat_bonuses:
		stat_bonus_other += comp_stat_other.get_stat(key)
	stat_bonus_other *= stat_bonuses_sign
	stat_bonus_other *= other_stat_bonuses_multiplier
